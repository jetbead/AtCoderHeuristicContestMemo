...
static constexpr double TIME_LIMIT = 1.97;
using Score = int;
Timer timer;
...
class TransitionSelector {
   ...
   public:
    ...
    inline Score diff() {
        return visit([](auto& x) -> Score { return x.diff(); }, transitions[select_index]);
    }
...
void solve() {
    ...
    // 探索
    const double SA_TIME_LIMIT = TIME_LIMIT - timer.getSec();
    Timer sa_timer;
    sa_timer.start();
    double sec;
    int step = 0;
    while (true) {
        step++;
        sec = sa_timer.getSec();
        if (sec > SA_TIME_LIMIT) break;
        double T = calc_temp(sec, SA_TIME_LIMIT);

        selector.select_transition();

        Score prev_score = state.score;
        selector.apply();
        Score new_score = state.score;

        Score delta = new_score - prev_score;
        bool undo = false;
        if (delta < 0) {
    ...
    cerr << "Processed Time = " << timer.getSec() << endl;
}
...
bool try_apply(State& state, Score threshold) {
    Score new_score = 1;
    Score delta = new_score - state.score;
    if (delta < threshold) {
        return false;
    }
...
void solve() {
    ...
    while (true) {
        ...
        Score threshold = min<Score>(0, (Score)(T * log(frand())));
        if (try_apply(state, threshold)) {
            if (best_state < state) {
                best_state = state;
            }
        }
