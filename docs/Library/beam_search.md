...
- chokudaiサーチでは、軸(キー)をどう用意するか、いろいろバリエーションが考えられる
  - (`vector<Heap> beams;`の`beams[key]`的なイメージ)
  - 基本的なbeam searchだと「今」に対して「次」のみだが、chokudaiサーチだと軸でheapを分けて持っているので「今」から「次」だけでなく「次の次」やその先などにも展開できる
    - 大きな遷移(複数ターンをまとめて1遷移にする)を考えたとき、keyを「1遷移」にする以外にも、keyを「1ターン」にして、遷移先のターンの所に入れるみたいなことができる
